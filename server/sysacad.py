...
class SysacadSession(object):
    ...
    def __init__(self, base_url, session=None):
        ...
        if session:
            assert isinstance(
                session, requests.Session), 'Object session must be instance of requests.Session.'
    ...
    def login(self, legajo, password):
        ...
        response = self.session.post(
            url, data={'legajo': legajo, 'password': password})
    ...
    def examenes(self):
        response = self._get('examenes.asp')
        html = BeautifulSoup(response.text)
        data = self._data_from_table(html, ('fecha', 'materia', 'nota', 'especialidad', 'plan', 'codigo'))
        return data
